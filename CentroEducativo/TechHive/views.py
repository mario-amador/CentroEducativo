...
class ImpuestosListView(ListView):
    model = Impuestos
    template_name = 'Impuesto/impuesto_listar.html'
    context_object_name = 'impuestos'

class ImpuestosCreateView(CreateView):
    model = Impuestos
    form_class = ImpuestosForm
    template_name = 'Impuesto/impuesto_crear.html'
    success_url = reverse_lazy('impuesto_listar')

    def form_valid(self, form):
        response = super().form_valid(form)
        
        # Crear un registro en ImpuestoHistorico
        impuesto_historico = ImpuestoHistorico(
            impuesto=self.object,  # Asignar el nuevo objeto Impuestos
            fecha_inicio=timezone.now(),  # Usar la fecha y hora actual
            valor=self.object.valor,  # Asignar el nuevo valor
        )
        impuesto_historico.save()
        
        return response

class ImpuestosDetailView(DetailView):
    model = Impuestos
    template_name = 'Impuesto/impuesto_detalle.html'

class ImpuestosUpdateView(UpdateView):
    model = Impuestos
    form_class = ImpuestosForm
    template_name = 'Impuesto/impuesto_editar.html'
    success_url = reverse_lazy('impuesto_listar')

    def form_valid(self, form):
        response = super().form_valid(form)
        
        # Actualizar el registro anterior de ImpuestoHistorico si existe uno
        last_impuesto_historico = ImpuestoHistorico.objects.filter(impuesto=self.object).last()
        if last_impuesto_historico:
            last_impuesto_historico.fecha_final = timezone.now() - timedelta(days=1)  # Restar un día a la fecha actual
            last_impuesto_historico.save()
        
        # Crear un nuevo registro en ImpuestoHistorico
        impuesto_historico = ImpuestoHistorico(
            impuesto=self.object,  # El objeto Impuestos actualizado
            fecha_inicio=timezone.now(),  # Usar la fecha y hora actual
            valor=self.object.valor,  # Asignar el nuevo valor
        )
        impuesto_historico.save()
...
class ImpuestosDeleteView(DeleteView):
    model = Impuestos
    template_name = 'Impuesto/impuesto_eliminar.html'
    success_url = reverse_lazy('impuesto_listar')
...
class DescuentoListView(ListView):
    model = Descuento
    template_name = 'Descuento/descuento_listar.html'
    context_object_name = 'descuentos'

class DescuentoCreateView(CreateView):
    model = Descuento
    form_class = DescuentoForm
    template_name = 'Descuento/descuento_crear.html'
    success_url = reverse_lazy('descuento_listar')

class DescuentoDetailView(DetailView):
    model = Descuento
    template_name = 'Descuento/descuento_detalle.html'

class DescuentoUpdateView(UpdateView):
    model = Descuento
    form_class = DescuentoForm
    template_name = 'Descuento/descuento_editar.html'
    success_url = reverse_lazy('descuento_listar')

class DescuentoDeleteView(DeleteView):
    model = Descuento
    template_name = 'Descuento/descuento_eliminar.html'
    success_url = reverse_lazy('descuento_listar')

#views Impuesto Historico

class ImpuestoHistoricoListView(ListView):
    model = ImpuestoHistorico
    template_name = 'ImpuestoHistorico/impuestohistorico_listar.html'
    context_object_name = 'impuestoshistoricos'

class ImpuestoHistoricoCreateView(CreateView):
    model = ImpuestoHistorico
    form_class = ImpuestoHistoricoForm
    template_name = 'ImpuestoHistorico/impuestohistorico_crear.html'
    success_url = reverse_lazy('impuestohistorico_listar')

class ImpuestoHistoricoDetailView(DetailView):
    model = ImpuestoHistorico
    template_name = 'ImpuestoHistorico/impuestohistorico_detalle.html'

class ImpuestoHistoricoUpdateView(UpdateView):
    model = ImpuestoHistorico
    form_class = ImpuestoHistoricoForm
    template_name = 'ImpuestoHistorico/impuestohistorico_editar.html'
    success_url = reverse_lazy('impuestohistorico_listar')

class ImpuestoHistoricoDeleteView(DeleteView):
    model =ImpuestoHistorico
    template_name = 'ImpuestoHistorico/impuestohistorico_eliminar.html'
    success_url = reverse_lazy('impuestohistorico_listar')