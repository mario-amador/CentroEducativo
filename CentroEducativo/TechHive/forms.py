...
from .models import Alumno, DocumentoDPI,Empleado,Catedratico, Factura, Pago,Tutor,Asignatura,Matricula,Reportes,ExpedienteMedico, HorariosNivelEducativo, NivelEducativo, ParcialesAcademicos, NotasAlumnos, Municipio, TipoPago,Seccion, Actitud, CentroEducativo, TutoresAlumnos, Impuestos, Descuento, ImpuestoHistorico
...
from .models import Alumno, DocumentoDPI,Empleado,Catedratico,Tutor,Asignatura,Matricula,Reportes,ExpedienteMedico, HorariosNivelEducativo, NivelEducativo, ParcialesAcademicos, NotasAlumnos, Municipio, TipoPago, Seccion, Actitud, CentroEducativo, TutoresAlumnos, Impuestos, Descuento, ImpuestoHistorico
from typing import Required, Self
...
class ParentescoForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['Parentesco']
        labels = {
            'NombreImpuesto': 'Nombre del Impuesto',
...
class ImpuestosForm(forms.ModelForm):
    class Meta:
        model = Impuestos
        fields = ['NombreImpuesto', 'valor']
        labels = {
            'NombreImpuesto': 'Nombre del Impuesto',
            'valor': 'Valor del Impuesto',
        }
        widgets = {
            'NombreImpuesto': forms.TextInput(attrs={'class': 'form-control'}),
            'valor': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
        }

    def clean_NombreImpuesto(self):
        nombreimpuesto = self.cleaned_data['NombreImpuesto']
        
        if not re.match(r'^\d+%$', nombreimpuesto):
            raise forms.ValidationError("Solo debe contener números y el símbolo '%'.")
...
class DescuentoForm(forms.ModelForm):
    ...
    def clean_valor(self):
        ...
        if not valor:
            raise forms.ValidationError('No puede estar vacío.')
        return valor

class ImpuestoHistoricoForm(forms.ModelForm):
    class Meta:
        model = ImpuestoHistorico
        fields = ['impuesto', 'fecha_inicio', 'fecha_final', 'valor']
        labels = {
            'impuesto': 'Impusto',
            'fecha_inicio': 'Fecha de Inicio',
        ...
        widgets = {
            'impuesto': forms.Select(attrs={'class': 'form-control'}),
            'fecha_inicio': forms.DateTimeInput(attrs={'class': 'form-control'}),
            'fecha_final': forms.DateTimeInput(attrs={'class': 'form-control'}),
            'valor': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
        }
